 ...
 * This class is mutable, because we sometimes need to change it once it's
 * created.
 * Mutators: populateMineField, resetEmpty
 * Includes convenience method to tell the number of mines adjacent to a
 * location.
 */
public class MineField {
     ...
     * with the mines in
     * the array such that if mineData[row][col] is true, then hasMine(row,col) will
     * be true and vice
     * versa. numMines() for this minefield will corresponds to the number of 'true'
     * values in
     * mineData.
     * 
     ...
     * numMines mines (once
     * populateMineField is called on this object). Until populateMineField is
     * called on such a
     * MineField, numMines() will not correspond to the number of mines currently in
     * the MineField.
     * 
     ...
     *                 it.
     *                 PRE: numRows > 0 and numCols > 0 and 0 <= numMines < (1/3 of
     ...
     *            PRE: inRange(row, col) and numMines() < (1/3 * numRows() *
     *            numCols())
     */
     ...
     * numRows() or
     * numCols(). Thus, after this call, the actual number of mines in the minefield
     * does not match
     * numMines().
     * Note: This is the state a minefield created with the three-arg constructor is
     * in at the
     * beginning of a game.
     */
    ...
    /**
     * Returns the number of mines adjacent to the specified location (not counting
     * a possible
     * mine at (row, col) itself).
     * Diagonals are also considered adjacent, so the return value will be in the
     * range [0,8]
     * 
     ...
     * with the 3-arg
     * constructor, some of the time this value does not match the actual number of
     * mines currently
     * on the field. See doc for that constructor, resetEmpty, and populateMineField
     * for more
     * details.
     * 
